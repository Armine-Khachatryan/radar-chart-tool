...
function Quiz6() {
    ...
    const idSection6 = state.sectionsDataId;
    ...
    // console.log(idSection6, "idSection6");
    ...
    let getQuestionsData = async () => {
        setIsLoading(true);
        try {
            let response = await axios.get(`${config.baseUrl}/api/questions/${idSection6}`);
            setIsLoading(false);
    ...
    return (
        <div className="quizWhole">
            <Header/>
            <div>
                <div className="quizTitle">6/10 {questionsData[0]?.section_name[0]?.title}</div>
            ...
            <div className={"below"}>
                ...
                <div className={`${body?.length === 10 ? "iconStyle" : "iconStyleNotAllowed"}`}
                     onClick={() => {
                         if (body.length === 10) {
                             navigate('/emotional-performance', {
                                 state: {
