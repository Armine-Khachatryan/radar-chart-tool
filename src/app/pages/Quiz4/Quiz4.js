...
function Quiz4() {
    ...
    const idSection4 = state.sectionsDataId;
    ...
    let getQuestionsData = async () => {
        setIsLoading(true);
        try {
            let response = await axios.get(`${config.baseUrl}/api/questions/${idSection4}`);
            setIsLoading(false);
    ...
    return (
        <div className="quizWhole">
            <Header/>
            <div>
                <div className="quizTitle">4/10 {questionsData[0]?.section_name[0]?.title}</div>
