...
function Quiz8() {
    ...
    const idSection8 = state.sectionsDataId;
    ...
    let getQuestionsData = async () => {
        setIsLoading(true);
        try {
            let response = await axios.get(`${config.baseUrl}/api/questions/${idSection8}`);
            setIsLoading(false);
    ...
    return (
        <div className="quizWhole">
            <Header/>
            <div>
                <div className="quizTitle">8/10 {questionsData[0]?.section_name[0]?.title}</div>
