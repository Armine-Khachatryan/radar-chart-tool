...
function FinancialStress(props) {
    ...
    // console.log(state, "state financialStress")

    return (
        <div className="whole"
             style={{
                 'backgroundImage':
                     `url(${config.baseUrl}/bg_img/${props?.sectionsData[4]?.bg_img})`
             }}>
            <Header/>
            <div>
                <div className={classes.title}>{props?.sectionsData[4]?.title}</div>
                <div className={classes.text}>{props?.sectionsData[4]?.description}</div>
            </div>
            <div className={classes.arrows}>
                ...
                <div className={classes.icon}
                     onClick={() => navigate('/quiz-5', {
                         state: {
                             sectionsDataId: props?.sectionsData[4]?.id,
...
export default FinancialStress;
