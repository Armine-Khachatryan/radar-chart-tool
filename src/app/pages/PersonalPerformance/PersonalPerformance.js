...
function PersonalPerformance(props) {
    ...
    // console.log(state, "state personal performance")

    return (
        <div className="whole"
             style={{
                 'backgroundImage':
                     `url(${config.baseUrl}/bg_img/${props?.sectionsData[8]?.bg_img})`
             }}>
            <Header/>
            <div>
                <div className={classes.title} style={{maxWidth: "623px"}}>{props?.sectionsData[8]?.title}</div>
                <div className={classes.text}>{props?.sectionsData[8]?.description}</div>
            </div>
            <div className={classes.arrows}>
                ...
                <div className={classes.icon}
                     onClick={() => navigate('/quiz-9', {
                         state: {
                             sectionsDataId: props?.sectionsData[8]?.id,
