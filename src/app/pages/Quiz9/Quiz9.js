...
function Quiz9() {
    ...
    const idSection9 = state.sectionsDataId;
    ...
    let getQuestionsData = async () => {
        setIsLoading(true);
        try {
            let response = await axios.get(`${config.baseUrl}/api/questions/${idSection9}`);
            setIsLoading(false);
    ...
    return (
        <div className="quizWhole">
            <Header/>
            <div>
                <div className="quizTitle">9/10 {questionsData[0]?.section_name[0]?.title}</div>
