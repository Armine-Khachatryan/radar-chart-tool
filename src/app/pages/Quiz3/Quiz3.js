...
function Quiz3() {
    ...
    let getQuestionsData = async () => {
        setIsLoading(true);
        try {
            let response = await axios.get(`${config.baseUrl}/api/questions/${idSection3}`);
            setIsLoading(false);
    ...
    return (
        <div className="quizWhole">
            <Header/>
            <div>
                <div className="quizTitle">3/10 {questionsData[0]?.section_name[0]?.title}</div>
